...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file'])) {
    ...
    $album_id = isset($_POST['albumId']) && !empty($_POST['albumId']) ? intval($_POST['albumId']) : NULL; // Get selected album_id
        ...
        if ($fileError === UPLOAD_ERR_OK) {
            if (move_uploaded_file($fileTmpPath, $filePath)) {
                ...
                if (!empty($type)) {
                    ...
                    // Insert file into the database with album_id
                    $stmt = $conn->prepare("INSERT INTO files (filename, filepath, filetype, size, dateupload, description, tag_id, album_id) VALUES (?, ?, ?, ?, NOW(), ?, ?, ?)");
                    if ($stmt) {
...
<div class="mb-3">
    ...
    <select class="form-control" id="albumId" name="albumId">
        <option value="">Select Album</option> <!-- Placeholder remains unchanged -->
        <?php
        ...
        if ($albumResult->num_rows > 0) {
            while ($album = $albumResult->fetch_assoc()) {
                echo '<option value="' . htmlspecialchars($album['id']) . '">ID: ' . htmlspecialchars($album['id']) . ' - ' . htmlspecialchars($album['name']) . '</option>';
            }
        } else {
