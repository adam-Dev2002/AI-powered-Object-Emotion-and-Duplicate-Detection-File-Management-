...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file'])) {
    ...
    $albumId = $_POST['albumId'] ?? null;  // Retrieve the album ID from the form
    ...
    // Retrieve the album table name based on the album ID
    if ($albumId) {
        $albumQuery = $conn->prepare("SELECT name FROM albums WHERE id = ?");
        $albumQuery->bind_param("i", $albumId);
        $albumQuery->execute();
        $albumResult = $albumQuery->get_result();
        if ($album = $albumResult->fetch_assoc()) {
            $albumTableName = "album_" . preg_replace('/[^A-Za-z0-9_]/', '', $album['name']); // Sanitize and format the table name
        } else {
            $response['status'] = 'error';
        ...
        $response['status'] = 'error';
        $response['message'] = "No album selected.";
        echo json_encode($response);
        exit;
    }
        ...
        if ($fileError === UPLOAD_ERR_OK) {
            if (move_uploaded_file($fileTmpPath, $filePath)) {
                ...
                if (!empty($type)) {
                    ...
                    // Insert file into the general files database
                    ...
                    if ($stmt) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            $albumInsertStmt = $conn->prepare("INSERT INTO `$albumTableName` (filename, filepath, filetype, upload_date) VALUES (?, ?, ?, NOW())");
                            $albumInsertStmt->bind_param("sss", $fileName, $fileType, $filePath);
                            if (!$albumInsertStmt->execute()) {
                                $response['status'] = 'error';
                                $response['message'] .= " Failed to insert into album-specific table.";
                            }
                            $albumInsertStmt->close();
                        } else {
                            $response['status'] = 'error';
...
document.getElementById('createAlbumForm').addEventListener('submit', function(event) {
    ...
    .then(data => {
        ...
        $('#createAlbumModal').modal('hide');  // Hide the modal after submission
    })
