...
// Function to start the Python script using exec()
function start_background_process($logFile) {
    $pythonPath = "/opt/homebrew/bin/python3.10"; // Use the correct Python 3.10 path
    $command = escapeshellarg($pythonPath) . " " . escapeshellarg(__DIR__ . "/yolo_ai.py") . " > " . escapeshellarg($logFile) . " 2>&1 & echo $!";
    exec($command, $output);
    return (int)$output[0];
}
