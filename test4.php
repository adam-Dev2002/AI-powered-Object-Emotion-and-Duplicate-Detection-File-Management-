...
<!-- Move To Button -->
<button type="button" class="btn btn-success" id="moveToBtn">Move To</button>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    moveBtn.addEventListener('click', function() {
        ...
        const selectedAlbumName = albumSelect.options[albumSelect.selectedIndex].text.split(' - ')[1]; // Extract album name
        ...
        return Promise.all(files.map(file => {
            return fetch('moveFile.php', {
                ...
                body: JSON.stringify({
                    ...
                    filename: file.name,
                    albumName: albumName,
                    albumId: albumId // Send albumId as an integer
                })
            })
            .then(response => {
...
document.addEventListener('click', async function(event) {
    if (event.target.classList.contains('delete-btn')) {
        const filePath = event.target.getAttribute('data-filepath'); // Ensure correct attribute retrieval
...
function deleteMedia(filePath, fileName) {
    ...
    // Fix: Ensure we don't attach multiple event listeners
    document.getElementById('confirmActionBtn').onclick = async function () {
        ...
        try {
            const response = await fetch('deleteMediaFile.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ filepath: filePath }) // Fix key name
            });

            const data = await response.json();

            if (data.status === 'success') {
                showSuccessModal(`"${fileName}" deleted successfully!`);
                setTimeout(() => location.reload(), 1000);
            } else {
...
function copyMedia(filePath, fileName) {
    ...
    document.getElementById('confirmationModalTitle').textContent = "Confirm Copy";
    document.getElementById('confirmationModalBody').innerHTML = `Are you sure you want to copy "<strong>${fileName}</strong>"?`;

    let confirmationModal = new bootstrap.Modal(document.getElementById('confirmationModal'));
    confirmationModal.show();

    document.getElementById('confirmActionBtn').textContent = "Copy";
    document.getElementById('confirmActionBtn').onclick = function () {
        fetch('copy_file.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
